 ...
 * @author juanfd
 */
class AutorizadoController extends Fmo_Controller_Action_Abstract
{
    private $_crear  = '/default/autorizado/crear';
    private $_masivo = '/default/autorizado/cargamasiva';
    ...
    {
        $this->_bootstrap();
        $form    = new Application_Form_Autorizado();
        ...
        if ($request->isPost()) {
            ...
            try {
                if ($form->isValid($post)) {
                    ...
                    $this->addMessageSuccessful('Ficha ' . $post[Application_Form_Autorizado::E_FICHA] . ' agregada como autorizado.');
                ...
                switch ($e->getCode()) {
                    case 23505:
                        $msj = 'La ficha <b>' . $post[Application_Form_Autorizado::E_FICHA] . '</b> ya está registrada como autorizado.';
                        break;
                    default:
                        $msj = $e->getMessage();
                        break;
    ...
    public function cargamasivaAction()
    {
        $this->_bootstrap();
        $res  = null;
        $form = new Application_Form_AutorizadoMasivo();
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($form->isValid($post)) {
                try {
                    if ($this->getParam(Application_Form_AutorizadoMasivo::E_CARGAR_ARCHIVO)) {
                        if ($form->getElement(Application_Form_AutorizadoMasivo::E_ARCHIVO)->getValue()) {
