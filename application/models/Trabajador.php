 ...
 * @author juanfd
 */
class Application_Model_Trabajador
{
    ...
    public static function esInactivo($cedula)
    {
        ...
        $personal   = new Fmo_Model_Personal();
        $trabajador = $personal->addFilterByCedula($cedula)->findOne();
        return in_array($trabajador->id_actividad, $inactivos);
    }
    ...
    public static function tieneDireccion($cedula)
    {
        $direccion = count(Application_Model_DbTable_Direccion::findOneByColumn('cedula', $cedula));
        return $direccion > 0;
    }
}